...
class Smartbill
{
    ...
    public function createInvoice(array $stripe_invoice, array $stripe_charge): array
    {
        ...
        $smartbill_invoice = [
            ...
            'client' => [
                'address' => $stripe_invoice['customer_address']['line1'] . ' ' . $stripe_invoice['customer_address']['line2'],
        ...
        if (!empty($stripe_invoice['customer_tax_ids']['0']['type'])
        ...
        if ($stripe_invoice['customer_tax_exempt'] === 'none') {
            ...
            if (!isset(self::VAT_RATES[$customer_country])) {
                throw new \Exception('Country ' . $customer_country . ' not defined in Smarbill::VAT_RATES rates, please define.');
            }
